...
temp_img = cv2.imread("../dataset/landcover/images/M-34-51-C-d-4-1.tif") #3 channels / spectral bands
plt.imshow(temp_img[:,:,0]) #View each channel...
print(temp_img[:,:,0])
temp_mask = cv2.imread("../dataset/landcover/masks/M-34-51-C-d-4-1.tif") #3 channels but all same.
...
root_directory = '../dataset/landcover/'
        ...
        if image_name.endswith(".tif"):
            ...
            for i in range(patches_img.shape[0]):
                for j in range(patches_img.shape[1]):
                    ...
                    # cv2.imshow("image", single_patch_img)
                    # cv2.waitKey(500)
                    # cv2.destroyAllWindows()

                    cv2.imwrite(root_directory+"256_patches/images/" + image_name.split(".")[0]+"patch_"+str(i)+str(j)+".tif", single_patch_img)
                    print(root_directory+"256_patches/images/" + image_name.split(".")[0]+"patch_"+str(i)+str(j)+".tif")
        ...
        if mask_name.endswith(".tif"):           
            ...
            for i in range(patches_mask.shape[0]):
                for j in range(patches_mask.shape[1]):
                    ...
                    cv2.imwrite(root_directory+"256_patches/masks/"+ mask_name.split(".")[0]+"patch_"+str(i)+str(j)+".tif", single_patch_mask)



train_img_dir = "../dataset/landcover/256_patches/images/"
train_mask_dir = "../dataset/landcover/256_patches/masks/"
...
for img in range(len(img_list)):   #Using t1_list as all lists are of same size
    ...
    if (1 - (counts[0]/counts.sum())) > 0.05:  #At least 5% useful area with labels that are not 0
        print("Save Me")
        cv2.imwrite('../dataset/landcover/256_patches/images_with_useful_info/images/'+img_name, temp_image)
        cv2.imwrite('../dataset/landcover/256_patches/images_with_useful_info/masks/'+mask_name, temp_mask)
...
input_folder = '../dataset/landcover/256_patches/images_with_useful_info/'
output_folder = '../dataset/landcover/data_for_training_and_testing/'
# Split with a ratio.
